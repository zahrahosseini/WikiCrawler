...
public class WikiTennisRanker {
	...
	public static void main(String[] args) {
		...
		WikiTennisRanker wiki = new WikiTennisRanker();
		...
		System.out.println("Jacc Sim(topRank,topInDegree)= "
				+ (double) wiki.intersection(list1, list2).size() / wiki.union(list1, list2).size());

		System.out.println("Jacc Sim(topRank,topOutDegree)= "
				+ (double) wiki.intersection(list1, list3).size() / wiki.union(list1, list3).size());

		// System.out.println(wiki.intersection(list3, list2));
		System.out.println("Jacc Sim(topInDegree,topOutDegree)= "
				+ (double) wiki.intersection(list2, list3).size() / wiki.union(list2, list3).size());


		// Calculate with e=0.005
		pr = new PageRank("WikiTennisGraph.txt", 0.005);
		...
		//Calculate JACC SIM for each pairs
		System.out.println("Jacc Sim(topRank,topInDegree)= "
				+ (double) wiki.intersection(list1, list2).size() / wiki.union(list1, list2).size());
